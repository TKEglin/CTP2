...
class TCP_Server:
    ...
    def listener(self, TCPsocket: socket):
        print("\nListening...")
        while True:
        ...
        if(isinstance(message, str)):
            ...
            match message_components[0]:
                ...
                case "longest_unwatched_timestamp":
                    ...
                    self.update_db_timestamp(cursor, timestamp)
                    database_connection.commit()
        else:  
            ...
            match event.event_type_enum:
                case HEvent.SystemOff.value:
                    ...
                    statuscolor = "gray"
                    self.update_db_timestamp(cursor, -1)
                    self.reset_room_data(database_connection, cursor)
                case (HEvent.SystemOn.value      | 
                    HEvent.SystemRestart.value):
                    status = "System running | No devices in use"
                    statuscolor = "teal"
                    self.update_db_timestamp(cursor, -1)
                    self.initialize_room_data(database_connection, cursor)
    ...
    def initialize_room_data(self, db_connection, cursor):
